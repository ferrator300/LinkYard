...
abstract class Controller
{
	...
	public function renderView($error = null)
	{
		...
		if ($error == "ERROR") {
			...
			$template = $twig->loadTemplate($this->twig);
			...
			$twig->getExtension('core')->setTimezone('Europe/Andorra');
			$template = $twig->loadTemplate($this->controller->twig);
			$HTMLCOS = $template->render($this->controller->data);
		}

		//preparem i carregauem la interfície Layout
		$loader2 = new Twig_Loader_Filesystem(DIRBASE . "/vendors/me");
		$twig2 = new Twig_Environment(
			$loader2,
			array(
				//'cache' => './cache',
				'cache' => false,
		...
		$tempUser = new UsersManager();
		if ($tempUser->restoreUser())
			$DATA['USER'] = true;
		else
			$DATA['USER'] = false;
		...
		$template = $twig2->loadTemplate("layout.html");
